...
self:restoreLimbProperties(part)
    ...
    -- === FIX FLOTACIÓN AL BAJARSE DE MOTOCROSS / VEHÍCULO ===
    if humanoid then
        humanoid.Seated:Connect(function(isSeated, currentSeat)
            if not isSeated then -- El jugador se acaba de bajar del asiento
                task.wait(0.2) -- Espera un poco para que termine la animación de desmonte
               
                local root = char:FindFirstChild("HumanoidRootPart")
                if root and root.Parent then
                    -- Refrescamos la física y posición del RootPart
                    root.CFrame = root.CFrame + Vector3.new(0, 0.05, 0) -- pequeño ajuste vertical
                   
                    -- Pequeño impulso para forzar resincronización
                    root.AssemblyLinearVelocity = Vector3.new(0, 1, 0)
                   
                    -- Restauramos y reaplicamos el tamaño del limb grande (importante)
                    task.wait(0.08)
                    for limb, data in pairs(parent._limbStore or {}) do
                        if limb and limb.Parent and data.OriginalSize then
                            limb.Size = data.OriginalSize -- volvemos a tamaño normal temporalmente
